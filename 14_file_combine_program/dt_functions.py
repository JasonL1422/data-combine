...
def combine_csv(output_filename=None):
    ...
    files = sorted(glob.glob('Upload/*.csv'))
    ...
    nn = os.path.splitext(os.path.basename(files[0]))[0]  # remove .csv extension
...
def combine_txt(output_filename=None):
    ...
    files = sorted(glob.glob('Upload/*.txt'))
    combined_df = pd.DataFrame()
    ...
    nn = os.path.splitext(os.path.basename(files[0]))[0] #remove extension
    tt = datetime.now().strftime("%m%d_%S")
